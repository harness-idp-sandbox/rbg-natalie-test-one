...
# run app
java -jar target/natalie-test-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/natalie-test-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/natalie-test-one -n dev

# get external endpoint
kubectl get svc natalie-test-one-svc -n dev
```
...
```bash
kubectl port-forward svc/natalie-test-one-svc 8080:80 -n dev
...
kubectl rollout restart deploy/natalie-test-one -n dev
kubectl rollout status deploy/natalie-test-one -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/natalietestone/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/natalietestone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
